  ...
  worker_online.on('online', function(worker) {
    if (i_am_the_master) {
      worker.on('message', function(msg) {
        if (msg.log && msg.logger_id && loggers[msg.logger_id] && !process.env.DISABLE_LOG4NODE_IPC) {
  ...
  if (config.parent) {
    ...
    if (cluster.isMaster) {
      if (config.file === undefined) {
        ...
        this.reopen();
        worker_online.on('online', function(worker) {
          if (process.env.DISABLE_LOG4NODE_IPC) {
      ...
      master_message_listener.on('message', function(msg) {
        if (msg.file && msg.fd && msg.file === this.file) {
          this.reopen(msg.fd);
        }
      }.bind(this));
    }
...
Log4Node.prototype.reopen = function(fd) {
  ...
  if (cluster.isMaster && process.env.DISABLE_LOG4NODE_IPC) {
    this.stream.once('open', function(fd) {
      Object.keys(cluster.workers).forEach(function(id) {
        cluster.workers[id].send({file: this.file, fd: fd});
      }.bind(this));
    }.bind(this));
  }
}
