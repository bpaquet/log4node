...
exports.init = function(lvl, fname){
  //support string or integer
  if (typeof lvl === 'string') {
    if (exports[lvl.toUpperCase()] === undefined) {
      throw new Error(lvl + 'is not used by the system');
    }
  ...
  if (cluster.isMaster) {
    if (fname) {
      filename = fname;
      reopen();
      //be sure to wait until the end of the file creation
...
exports.emergency =  function(msg){
  log('EMERGENCY', format(arguments));
};

exports.alert = function(msg){
  log('ALERT', format(arguments));
};

exports.critical = function(msg){
  log('CRITICAL', format(arguments));
};

exports.notice = function(msg){
  log('NOTICE', format(arguments));
};
