...
function Log4Node(level, filename, parentLogger) {
  this.setLogLevel(level);
  //if we want to use the same file but with different , we clone the logger.
  if (parentLogger) {
    if (parentLogger === "default") {
      parentLogger = defaultLogger;
    }

    this.id = parentLogger.id;
    this.stream = parentLogger.stream;
    this.filename = parentLogger.filename;
    return;
  }
