...
function Log4Node(config) {
  ...
  if (config.parent) {
    ...
    var parent_prefix = this.parent.prefix;
    if (typeof this.parent.prefix === 'string') {
      parent_prefix = function(level) {
        return this.computePrefix(this.parent.prefix, level);
      }.bind(this);
    }
    if ((config.prefix === undefined) || (typeof config.prefix === 'string')) {
      prefix = function(level) {
        return this.computePrefix(config.prefix || '', level);
      }.bind(this);
    }
    this.prefix = function(level) {
      return parent_prefix(level) + prefix(level);
    }
...
Log4Node.prototype.log = function(level, args) {
  if (levels[level] <= this.level) {
    ...
    if (typeof this.prefix === 'function') {
      ...
      msg = this.computePrefix(this.prefix, level);
    }
