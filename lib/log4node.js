...
/**
 * create a new logger from another instance
 * especially designed for the submodules
 * @param (optional) prefix append a prefix to the parent prefix
 * @param (optional) level change the level of the logger
 * @return a copy of the logger with another prefix;
 */
Log4Node.prototype.clone = function (prefix, level) {
  var logLevel = level !== undefined ? level : this.level;
  var logPrefix = prefix !== undefined ? this.prefix + prefix : this.prefix;

  var logger = new Log4Node(logLevel);

  logger.id = this.id;
  logger.stream = this.stream;
  logger.filename = this.filename;
...
exports.reconfigure = function(level, filename) {
  return defaultLogger = new Log4Node(level, filename);
}
