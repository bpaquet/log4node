...
function Log4Node(config) {
  ...
  if (config.parent) {
    ...
    this.json = config.json === undefined ? !!this.parent.json : !!config.json;
    this.setLogLevel(config.level || this.parent.level);
    var child_prefix = config.prefix || '';
    this.prefix = function(level) {
      if (this.json) {
        var prefix = this.computePrefix(this.parent.prefix, level),
            childPrefix = this.computePrefix(child_prefix, level);
        Object.keys(childPrefix).forEach(function(key) {
          prefix[key] = childPrefix[key];
        });
        return prefix;
      } else {
        return this.computePrefix(this.parent.prefix, level) + this.computePrefix(child_prefix, level);
      }
    }.bind(this);
...
Log4Node.prototype.computePrefix = function(prefix, level) {
  if (typeof prefix === 'function') {
    return prefix(level);
  }
  else {
    if (this.json) {
      return {date: new Date().toUTCString(), log_level: level.toUpperCase(), pid: process.pid};
    } else {
      return prefix
...
Log4Node.prototype.log = function(level, args) {
  if (levels[level] <= this.level) {
    var msg = this.computePrefix(this.prefix, level);
    ...
    if (this.json) {
      if (typeof args[0] === 'object') {
        ...
        msg.message = util.format.apply(this, args);
      }
